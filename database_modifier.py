...
def alter_table_names():
    for db in dev_databases:
        ...
        for table in db_specfic_mapper.keys():
            cursor.execute(
                f'ALTER TABLE "{table}" RENAME TO {db_specfic_mapper[table]}'
            )
...
def alter_columns_names():
    for db in dev_databases:
        ...
        for table in db_specfic_mapper.keys():
            ...
            inserting_tb_name = [
                f'"{table_mapper[column[1]]}"' for column in columns
            ]

            old_colums_names = [f'"{column[1]}"' for column in columns]

            old_colums_names_stringfied = ", ".join(old_colums_names)

            inserting_tb_name = ", ".join(inserting_tb_name)
            inserting_tb_name = (
                f'INSERT INTO "{table}" ({inserting_tb_name})\n'
...
ignored_files = [
    ...
    "predict_dev.json",
    "predict_dev__.json",
    "predict_dev_perfect.json",
    "predict_dev_cols.json",
    "dev_gold_cols.sql",
]
